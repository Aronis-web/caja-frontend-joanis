param(
  ...
  [string]$Etiqueta = 'Compra QR',
  [ValidateSet('none','0','1')][string]$DataAdic = 'none'
)
...
$lf = Join-Path $dir 'qr-login.json'
...
if ($DataAdic -ne 'none') { $req.ecr_data_adicional = $DataAdic }
$rf = Join-Path $dir 'qr-req.json'
($req | ConvertTo-Json -Compress) | Out-File -FilePath $rf -Encoding ascii -NoNewline

Write-Host ""
Write-Host "===== $Etiqueta - S/ $Amount (data_adic=$DataAdic) ====="
Write-Host "Body: $(Get-Content $rf)"
Write-Host ""

$sw = [System.Diagnostics.Stopwatch]::StartNew()
$outFile = Join-Path $dir 'qr-resp.txt'

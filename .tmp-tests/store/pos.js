...
const paymentFlow_1 = require("@/utils/paymentFlow");
...
const persistCart = (state) => {
    if (state.cartItems.length === 0 && state.cartPayments.length === 0) {
        void async_storage_1.default.removeItem(CART_STORAGE_KEY);
        return;
    }
    const payload = {
    ...
    void async_storage_1.default.setItem(CART_STORAGE_KEY, JSON.stringify(payload));
};
const normalizeTaxRate = (taxType) => (taxType === 'GRAVADO' ? 18 : 0);
const normalizeTopSellerProduct = (product) => {
    const price = typeof product.price === 'number' ? product.price : (product.salePriceCents || 0) / 100;
...
const toDataUrlFromBlob = (blob, fallbackType = 'image/jpeg') => new Promise((resolve, reject) => {
...
const preloadImageAsDataUrl = async (url) => {
    if (!url)
        return undefined;
...
const persistTopSellers = async (products, cashRegisterId, salesCounter = 0) => {
    const now = new Date().toISOString();
    await async_storage_1.default.setItem(TOP_SELLERS_STORAGE_KEY, JSON.stringify(products));
    await async_storage_1.default.setItem(TOP_SELLERS_META_STORAGE_KEY, JSON.stringify({
        cashRegisterId,
        updatedAt: now,
        salesCounter,
    }));
...
const initializeStore = async () => {
    try {
        const storedCashRegister = await async_storage_1.default.getItem(STORAGE_KEY);
...
exports.usePOSStore = (0, zustand_1.create)((set, get) => ({
    ...
    setSelectedCashRegister: async (cashRegister) => {
        ...
        if (cashRegister) {
            await async_storage_1.default.setItem(STORAGE_KEY, JSON.stringify(cashRegister));
    ...
    loadCashRegistersBySite: async (siteId) => {
        try {
            ...
            const cashRegisters = await POSService_1.posService.getCashRegistersBySite(siteId);
            set({ isLoading: false });
    ...
    openSession: async (cashRegisterId, userId, openingBalance, notes) => {
        try {
            ...
            const session = await POSService_1.posService.openSession({
                cashRegisterId,
                userId,
                openingCashCents,
                notes,
            ...
            void (async () => {
                try {
                    const { useOfflineStore } = await Promise.resolve().then(() => __importStar(require('@/store/offline')));
                    ...
                    const { offlineSyncService } = await Promise.resolve().then(() => __importStar(require('@/services/OfflineSyncService')));
    ...
    closeSession: async (sessionId, closingBalance, notes) => {
        try {
            ...
            const session = await POSService_1.posService.closeSession(sessionId, {
                closingCashCents,
                notes,
            });
    ...
    loadActiveSession: async (cashRegisterId) => {
        try {
            set({ isLoading: true, error: null });
            const session = await POSService_1.posService.getActiveSession(cashRegisterId);
            // Guardar sesión en AsyncStorage para uso del servicio
            await async_storage_1.default.setItem(SESSION_STORAGE_KEY, JSON.stringify(session));
            set({ currentSession: session, isLoading: false });
            void get().refreshTopSellersInBackground(cashRegisterId, 40);
    ...
    refreshSession: async () => {
        ...
        if (currentSession) {
            try {
                const session = await POSService_1.posService.getSession(currentSession.id);
    ...
    loadPaymentMethods: async () => {
        try {
            ...
            const warehouseId = currentSession?.cashRegister?.site?.warehouseId || selectedCashRegister?.site?.warehouseId;
            console.log('💳 Loading payment methods with warehouseId:', warehouseId);
            const paymentMethods = await POSService_1.posService.getPaymentMethods(warehouseId);
            console.log('💳 Payment methods loaded:', paymentMethods.length);
            await async_storage_1.default.setItem(PAYMENT_METHODS_STORAGE_KEY, JSON.stringify(paymentMethods));
            ...
            try {
                const cached = await async_storage_1.default.getItem(PAYMENT_METHODS_STORAGE_KEY);
                if (cached) {
                    const paymentMethods = JSON.parse(cached);
    ...
    addItemToCart: (product, quantity) => {
        ...
        if (existingIndex >= 0) {
            ...
            const cappedQty = typeof availableStock === 'number' ? Math.min(desiredQty, availableStock) : desiredQty;
            ...
            // Add new item
            const cappedQty = typeof availableStock === 'number' ? Math.min(quantity, availableStock) : quantity;
            const newItem = {
    ...
    createSale: async (customerId, _documentType = '03', notes) => {
        ...
        const totalCents = (0, paymentFlow_1.toCents)(total);
        const paymentsTotalCents = (0, paymentFlow_1.toCents)(paymentsTotal);
        ...
        try {
            ...
            const payments = (0, paymentFlow_1.buildSalePayments)(total, cartPayments, paymentMethods);
            ...
            const response = await POSService_1.posService.createSale(currentSession.id, requestData);
    ...
    loadTopSellers: async (cashRegisterId, limit = 40) => {
        const state = get();
        const registerId = cashRegisterId || state.selectedCashRegister?.id || state.currentSession?.cashRegisterId;
        if (!registerId) {
            return;
        }
        set({ isTopSellersLoading: true });
        try {
            const products = await POSService_1.posService.getTopSellers(registerId, limit);
            const withCachedImages = await Promise.all(products.map(async (product) => {
            ...
            const cached = await async_storage_1.default.getItem(TOP_SELLERS_STORAGE_KEY);
            if (cached) {
                try {
    ...
    resetTopSellersState: () => {
        ...
        void async_storage_1.default.removeItem(TOP_SELLERS_STORAGE_KEY);
        void async_storage_1.default.removeItem(TOP_SELLERS_META_STORAGE_KEY);
    },
    // Utility
    ...
    initializeFromStorage: async () => {
        try {
            ...
            const sessionData = await async_storage_1.default.getItem(SESSION_STORAGE_KEY);
            if (sessionData) {
            ...
            const cachedPaymentMethods = await async_storage_1.default.getItem(PAYMENT_METHODS_STORAGE_KEY);
            if (cachedPaymentMethods) {
                try {
                    const paymentMethods = JSON.parse(cachedPaymentMethods);
                    set({ paymentMethods });
                }
                catch (parseError) {
            ...
            const cachedTopSellers = await async_storage_1.default.getItem(TOP_SELLERS_STORAGE_KEY);
            const cachedTopSellersMeta = await async_storage_1.default.getItem(TOP_SELLERS_META_STORAGE_KEY);
            if (cachedTopSellers) {
                try {
                    const topSellers = JSON.parse(cachedTopSellers).map(normalizeTopSellerProduct);
                    const meta = cachedTopSellersMeta
                        ? JSON.parse(cachedTopSellersMeta)
                        : undefined;
                    set({
                        topSellers,
            ...
            const cartData = await async_storage_1.default.getItem(CART_STORAGE_KEY);
            if (cartData) {
                try {
                    const persisted = JSON.parse(cartData);
                    const currentRegisterId = get().selectedCashRegister?.id ?? null;
                    const sameRegister = currentRegisterId === null || persisted.cashRegisterId === currentRegisterId;
                    if (sameRegister) {
                        set({
                        ...
                        await async_storage_1.default.removeItem(CART_STORAGE_KEY);
                    }
                }
                catch (parseError) {
    ...
    reset: () => {
        ...
        void async_storage_1.default.removeItem(STORAGE_KEY);
        void async_storage_1.default.removeItem(SESSION_STORAGE_KEY);
        void async_storage_1.default.removeItem(TOP_SELLERS_STORAGE_KEY);
        void async_storage_1.default.removeItem(TOP_SELLERS_META_STORAGE_KEY);
        void async_storage_1.default.removeItem(CART_STORAGE_KEY);
    },
...
initializeStore().then((initialState) => {
    if (initialState.selectedCashRegister) {
        exports.usePOSStore.setState(initialState);
    }
});

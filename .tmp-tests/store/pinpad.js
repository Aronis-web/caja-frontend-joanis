...
exports.usePinPadStore = void 0;
const zustand_1 = require("zustand");
const PinPadService_1 = require("@/services/PinPadService");
exports.usePinPadStore = (0, zustand_1.create)((set, get) => ({
    // Estado inicial
    status: 'DISCONNECTED',
    isAvailable: false,
    isProcessing: false,
    ...
    connect: async () => {
        ...
        try {
            ...
            await PinPadService_1.pinPadService.login();
            set({
    ...
    disconnect: () => {
        PinPadService_1.pinPadService.disconnect();
        set({
            status: 'DISCONNECTED',
            isAvailable: false,
            isAuthenticated: false,
    ...
    testConnection: async () => {
        ...
        try {
            const isAvailable = await PinPadService_1.pinPadService.testConnection();
            set({
                isAvailable,
    ...
    processSale: async (amountCents, currencyCode = '604') => {
        ...
        try {
            const response = await PinPadService_1.pinPadService.processSale(amountCents, currencyCode);
            set({
                isProcessing: false,
                status: 'CONNECTED',
    ...
    processSaleWithQR: async (amountCents, currencyCode = '604') => {
        set({ isProcessing: true, lastError: null, status: 'PROCESSING' });
        try {
            const response = await PinPadService_1.pinPadService.processSaleWithQR(amountCents, currencyCode);
            set({
                isProcessing: false,
                status: 'CONNECTED',
                lastTransaction: response,
    ...
    voidSale: async (referenceNumber) => {
        set({ isProcessing: true, lastError: null, status: 'PROCESSING' });
        try {
            const response = await PinPadService_1.pinPadService.voidSale(referenceNumber);
            set({
                isProcessing: false,
                status: 'CONNECTED',
                lastTransaction: response,
    ...
    reprint: async (referenceNumber) => {
        set({ isProcessing: true, lastError: null, status: 'PROCESSING' });
        try {
            const response = await PinPadService_1.pinPadService.reprint(referenceNumber);
            set({
                isProcessing: false,
                status: 'CONNECTED',
                lastTransaction: response,
    ...
    getDetailedReport: async () => {
        set({ isProcessing: true, lastError: null, status: 'PROCESSING' });
        try {
            const response = await PinPadService_1.pinPadService.getDetailedReport();
            set({
                isProcessing: false,
                status: 'CONNECTED',
                lastTransaction: response,
    ...
    getTotalsReport: async () => {
        set({ isProcessing: true, lastError: null, status: 'PROCESSING' });
        try {
            const response = await PinPadService_1.pinPadService.getTotalsReport();
            set({
                isProcessing: false,
                status: 'CONNECTED',
                lastTransaction: response,
    ...
    closeBatch: async () => {
        set({ isProcessing: true, lastError: null, status: 'PROCESSING' });
        try {
            const response = await PinPadService_1.pinPadService.closeBatch();
            set({
                isProcessing: false,
                status: 'CONNECTED',
                lastTransaction: response,
    ...
    updateConfig: (config) => {
        ...
        PinPadService_1.pinPadService.updateConfig(newConfig);
...
PinPadService_1.pinPadService.onStatusChange((status) => {
    exports.usePinPadStore.setState({ status });
});

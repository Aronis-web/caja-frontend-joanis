...
exports.offlineDatabase = void 0;
...
class OfflineDatabaseService {
    ...
    `);
        ...
        try {
            ...
            const sessionIdCol = rows.find((row) => row[1] === 'sessionId');
        ...
        this.db.run(`CREATE INDEX IF NOT EXISTS idx_sales_pending_reassign ON offline_sales(pendingReassignment)`);
     ...
     */
    async saveProducts(products, syncId) {
        if (!this.db)
            throw new Error('Database not initialized');
    ...
    `, [searchQuery, searchQuery, query, limit]);
        ...
        return results[0].values.map((row) => this.rowToProduct(row, results[0].columns));
    }
    /**
     ...
     */
    async markTokenPendingSync(token) {
        if (!this.db)
            throw new Error('Database not initialized');
    ...
    `, [
        ...
        this.saveToStorage();
        console.log(`✅ [OFFLINE_DB] Venta offline guardada: ${sale.localId}${sale.pendingReassignment ? ' (pending reassignment)' : ''}`);
    }
    /**
     ...
     */
    async reassignPendingSales(cashRegisterId, sessionId, sellerId, cashRegisterCode) {
        if (!this.db)
            throw new Error('Database not initialized');
        ...
        const countResult = this.db.exec(`SELECT COUNT(*) FROM offline_sales
       WHERE pendingReassignment = 1
          OR (syncStatus IN ('PENDING','FAILED') AND (sessionId IS NULL OR sessionId <> ?))`, [sessionId]);
        const count = countResult[0]?.values[0]?.[0] || 0;
        if (count === 0)
            return 0;
        this.db.run(`
    ...
    `, [cashRegisterId, cashRegisterCode || null, sessionId, sellerId, sessionId]);
        this.saveToStorage();
        console.log(`🔁 [OFFLINE_DB] ${count} ventas reasignadas a caja=${cashRegisterId} sesión=${sessionId}`);
        return count;
    }
    /**
     * Obtiene ventas pendientes de sincronización
     */
    async getPendingSales(limit = 10) {
        if (!this.db)
            throw new Error('Database not initialized');
        const results = this.db.exec(`
    ...
    `, [limit]);
        ...
        return results[0].values.map((row) => this.rowToSale(row, results[0].columns));
    }
    /**
     ...
     */
    async updateSaleSyncStatus(localId, status, serverData) {
        if (!this.db)
            throw new Error('Database not initialized');
        const now = new Date().toISOString();
        if (serverData) {
     ...
     */
    async saveUsersBundle(cashRegisterId, bundle) {
        if (!this.db)
            throw new Error('Database not initialized');
    ...
    `, [
        ...
        this.saveToStorage();
        console.log(`✅ [OFFLINE_DB] Bundle de usuarios guardado (${bundle.userCount} usuarios, expira ${bundle.expiresAt})`);
    }
    /**
     * Obtiene el bundle cifrado vigente para una caja.
     */
    async getUsersBundle(cashRegisterId) {
        if (!this.db)
            throw new Error('Database not initialized');
     ...
     */
    async clearUsersBundle(cashRegisterId) {
        if (!this.db)
            throw new Error('Database not initialized');
        this.db.run(`DELETE FROM users_bundle WHERE cashRegisterId = ?`, [cashRegisterId]);
        this.saveToStorage();
     ...
     */
    async getPendingLoginEvents(limit = 200) {
        if (!this.db)
            throw new Error('Database not initialized');
        const results = this.db.exec(`SELECT * FROM login_events WHERE syncStatus = 'PENDING' ORDER BY occurredAt ASC LIMIT ?`, [limit]);
        if (results.length === 0 || results[0].values.length === 0) {
            return [];
        }
     ...
     */
    async markLoginEventsSynced(ids) {
        if (!this.db)
            throw new Error('Database not initialized');
        if (ids.length === 0)
            return;
...
exports.offlineDatabase = new OfflineDatabaseService();

...
// eslint-disable-next-line @typescript-eslint/no-require-imports
const sut = require('./AppUpdatesService');
...
const assertEqual = (actual, expected, msg) => {
    assert(actual === expected, `${msg} (esperado: ${String(expected)}, recibido: ${String(actual)})`);
};
const resetCalls = () => {
    calls.length = 0;
};
const run = async () => {
    ...
    assert(checkUrl.startsWith(`${FAKE_API_URL}/api/pos/app-updates/check?`), `URL de check incorrecta: ${checkUrl}`);
    ...
    assert(checkUrl.includes('currentVersion=1.0.0'), 'check debe incluir currentVersion=1.0.0');
    ...
    assertEqual(calls[0].url, `${FAKE_API_URL}/api/pos/app-updates/latest/po%20s/windows`, 'URL latest debe encode-uri');
    ...
    assertEqual(calls[0].url, `${FAKE_API_URL}/api/pos/app-updates/releases/pos?platform=android`, 'releases con filtro');
    // 7) buildDownloadUrl()
    const dl = appUpdatesService.buildDownloadUrl('pos', 'android', '1.0.29');
    assertEqual(dl, `${FAKE_API_URL}/api/pos/app-updates/download/pos/android/1.0.29`, 'buildDownloadUrl debe usar svc-pos');

...
import * as cryptoOfflineReal from '../utils/cryptoOffline';
import type { DecryptedUsersBundle, OfflineLoginEvent } from '../types/offlineAuth';

// ============ Mocks compartidos (mutables entre suites) ============
const recordedEvents: OfflineLoginEvent[] = [];
const secureStorage = new Map<string, string>();
let argon2Accepted: Set<string> = new Set();
let currentBundle: DecryptedUsersBundle | null = null;
let deviceTokenValue: string | null = 'test-device-token';
...
const mocks: Record<string, unknown> = {
  ...
  '@/utils/secureStorage': {
    ...
    getSecureItem: async (k: string) => secureStorage.get(k) ?? null,
  ...
  './OfflineDatabase': {
    offlineDatabase: {
      initialize: async () => {},
      saveLoginEvent: async (e: OfflineLoginEvent) => {
  ...
  'hash-wasm': {
    argon2Verify: async ({ password, hash }: { password: string; hash: string }) =>
      argon2Accepted.has(`${password}|${hash}`),
  },
...
// eslint-disable-next-line @typescript-eslint/no-require-imports
const sut = require('./OfflineLoginService') as typeof import('./OfflineLoginService');
...
const makeBundle = (overrides: Partial<DecryptedUsersBundle> = {}): DecryptedUsersBundle => ({
  bundleId: 'bundle-1',
...
const reset = async () => {
  ...
  (offlineLoginService as any).attempts = new Map();
};
...
async function run() {
  ...
  lockedBundle.users[0].pinLockedUntil = new Date(Date.now() + 60_000).toISOString();
  ...
  assert(
    recordedEvents.some((e) => e.success && e.method === 'PASSWORD'),
    '6) evento de login success registrado'
  );
  ...
  await reset();
  currentBundle = makeBundle();
  let lastRes: Awaited<ReturnType<typeof offlineLoginService.verifyCredentials>> | null = null;
  for (let i = 0; i < 5; i++) {
    lastRes = await offlineLoginService.verifyCredentials({ ...baseParams, password: 'wrong' });
  }
  assert(
    lastRes !== null && !lastRes.ok && lastRes.reason === 'TOO_MANY_ATTEMPTS',
    '7) 5to intento → TOO_MANY_ATTEMPTS'
  );
  ...
  assert(
    !sixth.ok && sixth.reason === 'TOO_MANY_ATTEMPTS',
    '7) 6to intento sigue bloqueado pese a password correcto'
  );
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (offlineLoginService as any).currentSession = null;
  const restored = await offlineLoginService.restoreSession();
  assert(
    restored !== null && restored.user.email === 'cashier@test.com',
    '8) restoreSession devuelve sesión válida'
  );

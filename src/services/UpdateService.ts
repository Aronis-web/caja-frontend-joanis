...
interface UpdateLog {
  timestamp: string;
  event: 'check' | 'available' | 'download_start' | 'download_progress' | 'download_complete' | 'install' | 'rollback' | 'error' | 'crash_detected';
  version?: string;
...
export class UpdateService {
  ...
  private versionStateFile: string;
  private updateLogs: UpdateLog[] = [];
  private versionState: VersionState;
  private MAX_CRASH_ATTEMPTS = 2;
  private CRASH_DETECTION_TIMEOUT = 60000; // 1 minuto
   ...
   */
  private loadVersionState(currentVersion: string): VersionState {
    try {
  ...
  private saveVersionState(): void {
    try {
      fs.writeFileSync(
        this.versionStateFile,
        JSON.stringify(this.versionState, null, 2)
      );
    } catch (error) {
    ...
    try {
      if (fs.existsSync(this.logFile)) {
        ...
        this.updateLogs = lines
          ...
          .filter((log): log is UpdateLog => log !== null);
      }
    } catch (error) {
   ...
   */
  private detectCrashesOnStartup(): void {
   ...
   */
  log(event: UpdateLog['event'], details?: Record<string, any>): void {
   ...
   */
  private getLogPrefix(event: UpdateLog['event']): string {
    const prefixes: Record<UpdateLog['event'], string> = {
      check: '🔍',
      available: '📦',
      download_start: '⬇️',
      download_progress: '📊',
   ...
   */
  recordInstallationAttempt(version: string): void {
    this.versionState.updateAttempts++;
   ...
   */
  getTelemetryData(): {
    stats: ReturnType<typeof this.getUpdateStats>;
  ...
  cleanupOldLogs(daysToKeep: number = 30): void {
    ...
    try {
      const logContent = this.updateLogs
...
export function initializeUpdateService(appVersion: string): UpdateService {

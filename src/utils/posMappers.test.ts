...
import { cashCentsToSoles, mapOfflineProductToProduct } from './posMappers';
...
const makeOfflineProduct = (overrides: Partial<OfflineProduct> = {}): OfflineProduct => ({
  id: 'prod-1',
  sku: 'SKU-001',
...
const run = () => {
  ...
  assertEqual(cashCentsToSoles(0), 0, 'cashCentsToSoles(0) debe ser 0');
  assertEqual(cashCentsToSoles(100), 1, 'cashCentsToSoles(100) debe ser 1');
  assertEqual(cashCentsToSoles(12345), 123.45, 'cashCentsToSoles(12345) debe ser 123.45');
  assertEqual(cashCentsToSoles(-500), -5, 'cashCentsToSoles negativo debe respetar signo');

  // 2) cashCentsToSoles: null/undefined/NaN → 0 (regresión bug currentBalance)
  assertEqual(cashCentsToSoles(undefined), 0, 'undefined debe devolver 0');
  assertEqual(cashCentsToSoles(null), 0, 'null debe devolver 0');
  assertEqual(cashCentsToSoles(Number.NaN), 0, 'NaN debe devolver 0');
  assertEqual(
    cashCentsToSoles(Number.POSITIVE_INFINITY),
    0,
    'Infinity no es finito, debe devolver 0'
  );

  // 3) mapOfflineProductToProduct: campos requeridos preservados
  {
    const offline = makeOfflineProduct();
    const product = mapOfflineProductToProduct(offline);
  ...
  {
    const onlyBarcode = mapOfflineProductToProduct(
      makeOfflineProduct({ sku: null, barcode: '111' })
    );
    assertEqual(onlyBarcode.code, '111', 'code debe caer en barcode cuando sku es null');

    const onlySku = mapOfflineProductToProduct(makeOfflineProduct({ sku: 'ABC', barcode: null }));
  ...
  {
    const exonerado = mapOfflineProductToProduct(makeOfflineProduct({ taxType: 'EXONERADO' }));
    assertEqual(exonerado.taxRate, 0, 'taxRate=0 cuando EXONERADO');

    const inafecto = mapOfflineProductToProduct(makeOfflineProduct({ taxType: 'INAFECTO' }));
